...
public class DOMSaxon_EvalXPath
{
       public static void main(String[] args)
       {
           try
           {
               if (args.length != 2)
               {
                   System.err.println("Usage java RNDClass <xmlFileNameOrUrl> " +
               "<XPathExpression>\nExamples:");

            System.err.println("java RNDClass Options.xml " +
               "/options/option[@key='newFeedsUrl']/@value");

            System.err.println("java RNDClass Options.xml count(/options/option)");

            System.err.println("java RNDClass Options.xml " + "" +
               "\"concat(/options/option[position()=last()]/@key, '=', " +
               "/options/option[position()=last()]/@value)\"");

            System.err.println("java RNDClass http://www.PerfectXML.com/books.xml " +
               "count(/catalog/book)");
         ...
         RNDClass xpathEval = new RNDClass();
         System.out.println(xpathEval.evalXPath(args[0],args[1]));

       }  // end of main function

       public String showXPath(String xmlFileToLoad, String xpathExpr)
       {
           String result="";
